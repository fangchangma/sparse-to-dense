...
--
--
--  A slightly modified version of the AbsCriterion, where pixles with Nan values in the target will be ignored.
-- 
require 'nn'
local utils = require '../utils'

local myAbsCriterion, parent = torch.class('nn.myAbsCriterion', 'nn.AbsCriterion')

function myAbsCriterion:__init(sizeAverage)
   parent.__init(self) 
...
-- forward pass
function myAbsCriterion:updateOutput(input, target)
...
-- backward pass
function myAbsCriterion:updateGradInput(input, target)
   assert( input:nElement() == target:nElement(), "input and target size mismatch")
