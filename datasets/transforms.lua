...
local Max = nn.SpatialMaxPooling(2, 2, 2, 2, 0, 0)
...
function M.DepthNormalize(meanstd)
   return function(img)
      img = img:clone()
      img:add(-meanstd.mean)
      img:div(meanstd.std)
      return img
   end
end
...
function M.CenterCropRectangle(width, height)
   return function(input)
      return image.crop(input, 'c', width, height) -- center patch
   end
end
...
-- Crop
function M.Crop(w1, w2, h1, h2)
   return function(input)
      return image.crop(input, w1, h1, w2, h2) 
   end
end
...
function M.Rotation(deg)
   return function(input)
      if deg ~= 0 then
         -- note that binliear interpolation creates a problem when the input depth image is sparse
         -- input = image.rotate(input, (torch.uniform() - 0.5) * deg * math.pi / 180, 'bilinear')
         input = image.rotate(input, (torch.uniform() - 0.5) * deg * math.pi / 180, 'simple')
      end
      return input
   end
end
